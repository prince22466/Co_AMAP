...
def market_orders(obs,animal,crops,actions):
    ...
    unfed=sum(not t['fed_today'] for row in f['tiles'] for t in row if isinstance(t,dict) and 'animal' in t)
    feed_target=live if hour>=24 else unfed
    reserve_wheat=0 if day==29 else feed_target+0
