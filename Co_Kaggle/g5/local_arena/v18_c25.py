...
def fert_value(t,day,prices):
    ...
    first,last,cap={'WHEAT':(2,4,6),'CARROT':(2,3,4),'MELON':(6,10,6)}[c]
    if age<first or age>last:return 0
    days=min(last,age+2)-age+1
    room=max(0,cap-t['yield_units']-days)
    return min(days,room)*prices[c]-prices['FERTILIZER']
...
def unit_actions(obs,animal,crops):
    ...
    for p,c in crops.items():
        ...
        harvest=held>0 and ((ongoing and (held>=2 or day>=28)) or (not ongoing and age>=(4 if c=='WHEAT' and day<10 else CROPS[c][3])))
        ...
        if ongoing and age>=(4 if c=='WHEAT' and day<10 else CROPS[c][3]) and not held:
...
def market_orders(obs,animal,crops,actions):
    ...
    desired_hands=7 if len(f['unlocked_quadrants'])==1 else 10 if len(f['unlocked_quadrants'])==2 else 10
