...
def crop_plan(obs,projected,animal):
    ...
    wheat=set(opening[:19]);melon=set(opening[19:19])
    for p in points:
        t=tile(f,p)
...
def market_orders(obs,animal,crops,actions):
    ...
    if day==0 and hour==0:
        return [['BUY_SEED', 'WHEAT', 19], ['BUY_ANIMAL', 'COW', 2], ['BUY_ANIMAL', 'SHEEP', 2], ['BUY_PRODUCT', 'WHEAT', 4], ['HIRE'], ['HIRE'], ['HIRE'], ['HIRE'], ['HIRE']]
    ...
    if lands<3 and 2<=day<=15 and hour<18 and cash>landcost+500 and len(orders)<10:
