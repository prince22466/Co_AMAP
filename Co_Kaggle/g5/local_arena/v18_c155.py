...
def fert_value(t,day,prices):
    ...
    if age<first or age>last or t['watered_today']:return 0
...
def market_orders(obs,animal,crops,actions):
    ...
    for row in f['tiles']:
        for t in row:
            if isinstance(t,dict) and 'crop' in t and t['crop'] in ('TOMATO','STRAWBERRY') and fert_value(t,day,prices)>10:fert_need+=1
