...
def animal_plan(obs,projected):
    f=obs['farms'][obs['player']];plan={}
    for y,row in enumerate(obs['farms'][1-obs['player']]['tiles']):
        for x,t in enumerate(row):
            if isinstance(t,dict) and 'animal' in t and tile(f,(x,y))!='LOCKED' and obs['day']<=17:plan[(x,y)]=t['animal']
    for y,row in enumerate(f['tiles']):
        for x,t in enumerate(row):
            if isinstance(t,dict) and 'animal' in t:plan[(x,y)]=t['animal']
    stock=totals(obs['private'])
    for a in ANIMALS:
        available=stock.get(a,0)-sum(k==a and not (isinstance(tile(f,p),dict) and tile(f,p).get('animal')) for p,k in plan.items())
        for _ in range(max(0,available)):
            slots=[p for p in ANIMAL_POINTS if p not in plan and tile(f,p)!='LOCKED']
            if slots:plan[min(slots,key=lambda p:(dist(p,nearest_shed(p)),p))]=a
    return plan

OTHER_CROPS={}
def crop_plan(obs,projected,animal):
    global OTHER_CROPS
    if obs['day']==0 and obs['hour']==0:OTHER_CROPS={}
    for y,row in enumerate(obs['farms'][1-obs['player']]['tiles']):
        for x,t in enumerate(row):
            if isinstance(t,dict) and 'crop' in t:OTHER_CROPS[(x,y)]=t['crop']
    f=obs['farms'][obs['player']];day=obs['day'];seeds=obs['private']['seeds'];plan={}
    points=[(x,y) for y in range(10) for x in range(10) if tile(f,(x,y))!='LOCKED' and (x,y) not in animal]
    ...
    for p in points:
        ...
        if day<=3 and p in wheat|melon:
            ...
            c=max(scores)[1]
        mirror=OTHER_CROPS.get(p)
        if day>=3 and mirror and day+CROPS[mirror][3]<=29:c=mirror
        plan[p]=c
