...
def crop_plan(obs,projected,animal):
    ...
    wheat=set(opening[:15]);melon=set(opening[15:19])
    for p in points:
        t=tile(f,p)
...
def market_orders(obs,animal,crops,actions):
    ...
    if day==0 and hour==0:
        return [['HIRE'], ['HIRE'], ['HIRE'], ['HIRE'], ['HIRE'], ['HIRE'], ['BUY_SEED', 'WHEAT', 15], ['BUY_ANIMAL', 'COW', 2], ['BUY_ANIMAL', 'SHEEP', 2], ['BUY_SEED', 'MELON', 4]]
