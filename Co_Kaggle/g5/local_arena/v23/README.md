...
Do not commit the API key. If you pass `--api-key`, remember that command-line arguments may be visible in shell history or process listings on some systems. The replay subprocess still strips OpenAI/token/secret/password/credential environment variables before executing candidate code.
