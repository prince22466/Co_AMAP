...
def unit_actions(obs,animal,crops):
    ...
    for i,pos in enumerate(positions):
        ...
        cargo_value=sum(inv.get(c,0)*prices.get(c,0) for c in ('MILK','WOOL','MELON','STRAWBERRY','TOMATO','CARROT','EGG'))
        deliver=(goods>=3 and cargo_value>=500) or (tuple(pos) in SHED and goods>0) or (day<13 and (inv.get('MELON',0)>=6 or inv.get('WOOL',0)>=4))
    ...
    for p,c in crops.items():
        ...
        if harvest and (ongoing or t['watered_today'] or age>CROPS[c][3]):
            ...
            tasks.append((p,['HARVEST'],300+min(200,held*prices[c]/8)+deadline_bonus,None));continue
    ...
    for y,row in enumerate(f['tiles']):
        for x,t in enumerate(row):
            ...
            if t.get('yield_units',0):
                deadline_weight=V16_FINAL_BONUS if OPP_STYLE=='V16' else 40
                deadline_bonus=deadline_weight*dist(p,nearest_shed(p)) if day==29 else 0
                tasks.append((p,['HARVEST'],300+min(200,t['yield_units']*prices[ANIMALS[t['animal']][1]]/8)+deadline_bonus,None))
