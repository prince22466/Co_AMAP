...
def unit_actions(obs,animal,crops):
    ...
    live_points={p for p,op,_,_ in tasks if op[0] in ('FEED','CARE','COLLECT_FERTILIZER') or (op[0]=='HARVEST' and isinstance(tile(f,p),dict) and 'animal' in tile(f,p))}
    routes=(ROUTES[0],ROUTES[1],ROUTES[2]+ROUTES[3],ROUTES[4]+ROUTES[5])
    territories=[set(route)&live_points for route in routes]
    territories=[r for r in territories if r]
    herders=min(len(territories),len(positions))
    for i in list(free):
        if i>=herders or tuple(positions[i]) not in SHED:continue
        need=sum(not tile(f,p)['fed_today'] for p in territories[i])-invs[i].get('WHEAT',0)
        if need>0 and shed.get('WHEAT',0)>0:
            amount=min(need,shed['WHEAT']);shed['WHEAT']-=amount
            actions[i]=['PICKUP','WHEAT',amount];free.remove(i)
    ...
    while free and tasks:
        ...
        for i in free:
            for k,(p,op,weight,resource) in enumerate(tasks):
                if hour<24:
                    if i<herders:
                        working=any(t[0] in territories[i] for t in tasks)
                        if working and p not in territories[i] and op[0]!='PICKUP':continue
                    elif p in live_points or resource=='NO_WHEAT':continue
