...
# FILE MAP
#   1. Imports and configuration: shared engine, executor, and model helpers.
#   2. V21 agent: selling rules, executor integration, neural task selection.
#      The network class itself is QuantizedResidualQ in quantization.py.
#   3. Training rewards and experience-buffer sampling (not game replay).
#   4. Static game replay: new candidate decisions + recorded opponent commands.
#   5. Model initialization, Double-DQN updates, and checkpoint persistence.
#   6. Replay validation, CLI, and orchestration in main().
# The introductory docstring describes the original protocol; the current code
# also removes the tree prior, replaces selling, and uses dense worker rewards.
...
# ============================================================================
# 2. V21 AGENT: RULE-BASED EXECUTOR + NEURAL WORKER-TASK SELECTOR
# The full agent includes planners and selling rules; the neural network only
# scores generated worker-task candidates. Network implementation lives outside
# this file in quantization.py (QuantizedResidualQ).
# ============================================================================
...
# Rule-based market policies, not learned neural models.
...
# Connect the base executor to the Q selector and the active binary sell policy.
...
# Neural inference and exploration: construct candidates, score, choose a pair.
...
# Assemble the callable candidate agent used by the static replay loop.
...
# ============================================================================
# 3. TRAINING REWARDS AND EXPERIENCE BUFFER
# Convert production/transport/delivery into per-worker training credit.
# ReplayBuffer below means stored learning transitions, not recorded game replay.
# ============================================================================
...
# Experience-buffer sampling for gradient updates.
...
# ============================================================================
# 4. STATIC GAME REPLAY
# Recreate the environment from a saved history. Only opponent commands are
# fixed: candidate actions and all resulting game states are recomputed.
# ============================================================================
    ...
    *,
    ...
    try:
        for replay_step in range(1, len(history["steps"])):
            # V21 MODEL/AGENT: choose a fresh action from the current live state.
            ...
            # STATIC OPPONENT: reuse this turn's recorded opponent command.
            ...
            # GAME ENGINE: apply both actions and compute the next state.
            env.step(actions)

            # TRAINING DATA: attribute consequences to worker decisions.
...
# History discovery and train/validation split for static replay.
...
# ============================================================================
# 5. MODEL WEIGHTS, LEARNING UPDATES, AND CHECKPOINTS
# Load v20 weights as initialization; train the v21 tree-free neural scorer.
# ============================================================================
...
# Double-DQN learning: sample stored experience and update network parameters.
def q_update_v21(
...
# Persist/restore network, optimizer, and random-generator state.
...
# ============================================================================
# 6. VALIDATION AND RUN ORCHESTRATION
# Preflight checks replay fidelity; evaluation runs the candidate without
# exploration. main() connects model creation, rollout, learning, and saving.
# ============================================================================
...
def main():
    # A. Configure paths, history split, device, and random generators.
    ...
    # B. Decode parent v20 weights into reference networks.
    ...
    # C. CREATE THE V21 MODEL: online is trained; target supplies TD targets.
    # QuantizedResidualQ ignores the old tree prior despite its inherited name.
    online = QuantizedResidualQ(
    ...
    # D. Check recorded-game parity and parent-submission compatibility.
    ...
    # E. Set up learning state, optional resume, and baseline evaluation.
    ...
    # Training experience memory; this does not replay saved game commands.
    ...
    # F. Training iterations: static-game rollouts -> experience -> Q updates.
    for update in range(start_update, start_update + args.updates):
        ...
        for episode_index in range(args.episodes_per_update):
            ...
            # Run a newly simulated game against the recorded opponent stream.
