...
def crop_plan(obs,projected,animal):
    ...
    strawberry_count=sum(isinstance(tile(f,p),dict) and tile(f,p).get('crop')=='STRAWBERRY' for p in points)
    for p in points:
        t=tile(f,p)
        ...
        if day<=3 and p in wheat|melon:
            ...
            for c,(cost,base,events,last) in CROPS.items():
                if c=='STRAWBERRY' and strawberry_count>=26:continue
            ...
            c=max(scores)[1]
        plan[p]=c
        if c=='STRAWBERRY':strawberry_count+=1
