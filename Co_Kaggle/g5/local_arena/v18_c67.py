...
ANIMAL_POINTS={(4,4),(4,3),(4,2),(3,4),(3,3),(2,3)}|{(x,y) for x in range(5,9) for y in range(2,5)}|{(x,y) for x in range(2,5) for y in range(5,9)}
...
def animal_plan(obs,projected):
    ...
    base={(4,4):'COW',(3,4):'COW',(4,3):'SHEEP',(3,3):'SHEEP'}
    slots=sorted(ANIMAL_POINTS-set(base),key=lambda p:(dist(p,nearest_shed(p)),p))
    kinds=['COW']*(NC-2)+['SHEEP']*(NS-2)+['GOOSE']*NG
    base.update(dict(zip(slots,kinds)))
    for p,a in base.items():
        t=tile(f,p)
        if t=='LOCKED':continue
        if isinstance(t,dict) and t.get('animal'):plan[p]=t['animal']
        elif day<=17:plan[p]=a
    return plan
    ...
    for p,c in crops.items():
        ...
        if t is None:
            if hour<24 and available.get(c,0)>0:
                tasks.append((p,['PLANT',c],100 if day==0 else 200,None));available[c]-=1
            continue
        if not isinstance(t,dict):continue
