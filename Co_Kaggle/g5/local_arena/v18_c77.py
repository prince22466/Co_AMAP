...
def unit_actions(obs,animal,crops):
    ...
    zones=[(3,3),(4,4),(1,1),(1,3),(3,1)]
    if len(f['unlocked_quadrants'])>=2:zones += [(5,3),(7,3),(5,1),(8,1)]
    if len(f['unlocked_quadrants'])>=3:zones += [(3,5),(3,7),(1,5),(1,8)]
    homes=[zones[min(len(zones)-1,int(i*len(zones)/len(positions)))] for i in range(len(positions))]
    while free and tasks:
        ...
        for i in free:
            for k,(p,op,weight,resource) in enumerate(tasks):
                ...
                score=weight/(1+distance*.65+dist(homes[i],p)*6)
