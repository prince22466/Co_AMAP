...
def crop_plan(obs,projected,animal):
    ...
    wheat=set(opening[:10]);melon=set(opening[10:19])
    for p in points:
        t=tile(f,p)
...
def optimal_assignment(cost):
    n=len(cost)
    if not n:return []
    m=len(cost[0]);u=[0.]*(n+1);v=[0.]*(m+1);p=[0]*(m+1);way=[0]*(m+1)
...
def market_orders(obs,animal,crops,actions):
    ...
    if day==0 and hour==0:
        return [['BUY_SEED', 'WHEAT', 10], ['BUY_SEED', 'MELON', 9], ['BUY_ANIMAL', 'COW', 2], ['BUY_ANIMAL', 'SHEEP', 2], ['BUY_PRODUCT', 'WHEAT', 4], ['HIRE'], ['HIRE'], ['HIRE'], ['HIRE'], ['HIRE']]
    ...
    if lands<3 and 2<=day<=15 and hour<18 and cash>landcost+200 and len(orders)<10:
