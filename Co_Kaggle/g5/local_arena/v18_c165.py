...
def crop_plan(obs,projected,animal):
    ...
    for p in points:
        ...
        if day<=3 and p in wheat|melon:
            ...
            c=max(scores)[1]
        if p[0]>=5 and p[1]>=5 and day<=25:c=chr(87)+chr(72)+chr(69)+chr(65)+chr(84)
        plan[p]=c
...
def market_orders(obs,animal,crops,actions):
    ...
    desired_hands=7 if len(f['unlocked_quadrants'])==1 else 11 if len(f['unlocked_quadrants'])==2 else 12 if len(f["unlocked_quadrants"])==3 else 12
    ...
    lands=len(f['unlocked_quadrants']);landcost=1000*2**(lands-1)
    if lands<4 and 3<=day<=15 and hour<18 and cash>landcost+500 and len(orders)<10:
