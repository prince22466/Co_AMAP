...
# Tested with xlsx v0.65, data.table v.1.14.8, dplyr v1.1.4, lubridate v1.9.2
...
disp_max_dte <- as.Date(lppub_files %>% pull(AQSN_DTE) %>% max()) %m-% months(3)
...
aqsn_total <- lppub_files %>% 
  left_join(
    lppub_files %>% 
      group_by(ORIG_YR) %>% 
      summarize(wm.CSCORE_B = sum(CSCORE_B*orig_amt, na.rm = TRUE)/sum(if_else(is.na(CSCORE_B), 0, orig_amt)),
                wm.CSCORE_C = sum(CSCORE_C*orig_amt, na.rm = TRUE)/sum(if_else(is.na(CSCORE_C), 0, orig_amt)),
                wm.oltv = sum(oltv*orig_amt, na.rm = TRUE)/sum(if_else(is.na(oltv), 0, orig_amt)),
                wm.ocltv = sum(ocltv*orig_amt, na.rm = TRUE)/sum(if_else(is.na(ocltv), 0, orig_amt)),
                wm.dti = sum(dti*orig_amt, na.rm = TRUE)/sum(if_else(is.na(dti), 0, orig_amt)),
                wm.orig_rt = sum(orig_rt*orig_amt, na.rm = TRUE)/sum(if_else(is.na(orig_rt), 0, orig_amt))),
    by = "ORIG_YR"
  ) %>% summarize(LOAN_COUNT = n(),
                 SUM_orig_amt = sum(orig_amt)/1000000,
                 AVG_orig_amt = mean(orig_amt),
                 CSCORE_B = sum(wm.CSCORE_B*orig_amt)/sum(orig_amt),
                 CSCORE_C = sum(wm.CSCORE_C*orig_amt)/sum(orig_amt),
                 oltv = sum(wm.oltv*orig_amt)/sum(orig_amt),
                 ocltv = sum(wm.ocltv*orig_amt)/sum(orig_amt),
                 dti = sum(wm.dti*orig_amt)/sum(orig_amt),
                 orig_rt = sum(wm.orig_rt*orig_amt)/sum(orig_amt)
                 )
...
perf_stat <- lppub_files %>%
  ...
  summarize(
    ...
    D180_UPB = sum(F180_UPB, na.rm = TRUE) / 1000000,
    FCE_RT = sum(F180_UPB, na.rm = TRUE)/sum(orig_amt, na.rm = TRUE),
...
# Aggregate years 2000 through 2006
disp_loss <- lppub_files %>%
  filter(DISP_DTE == "" | as.Date(DISP_DTE) <= disp_max_dte) %>%
  mutate(
    DISP_YR = case_when(
      DISP_YR == '2000' ~ '2000-2006',
      DISP_YR == '2001' ~ '2000-2006',
      DISP_YR == '2002' ~ '2000-2006',
      DISP_YR == '2003' ~ '2000-2006',
      DISP_YR == '2004' ~ '2000-2006',
      DISP_YR == '2005' ~ '2000-2006',
      DISP_YR == '2006' ~ '2000-2006',
      TRUE ~ DISP_YR
    )
  ) %>% 
  group_by(DISP_YR) %>%
...
disp_loss_transpose <- disp_loss_transpose[2:nrow(disp_loss_transpose),1:ncol(disp_loss_transpose)]
colnames(disp_loss_transpose) <- c(disp_loss$DISP_YR[1:ncol(disp_loss_transpose)])

### Prepare the Origination Loss summary
# Aggregate years 1999 through 2005
orig_loss <- lppub_files %>%
  filter(DISP_DTE == "" | as.Date(DISP_DTE) <= disp_max_dte) %>% 
  mutate(
    ORIG_YR = case_when(
      ORIG_YR == '1999' ~ '1999-2005',
      ORIG_YR == '2000' ~ '1999-2005',
      ORIG_YR == '2001' ~ '1999-2005',
      ORIG_YR == '2002' ~ '1999-2005',
      ORIG_YR == '2003' ~ '1999-2005',
      ORIG_YR == '2004' ~ '1999-2005',
      ORIG_YR == '2005' ~ '1999-2005',
      TRUE ~ ORIG_YR
    )
  ) %>% 
  group_by(ORIG_YR) %>%
